...
function UIUtil.SetNewButtonStdTextures(btn, filename)
    btn:SetNewTextures(
        UIUtil.SkinnableFile(filename .. "_btn_up.dds"),
        UIUtil.SkinnableFile(filename .. "_btn_down.dds"),
        UIUtil.SkinnableFile(filename .. "_btn_over.dds"),
        UIUtil.SkinnableFile(filename .. "_btn_dis.dds")
    )
    btn:ApplyTextures()
end
...
function SwapButton(parent)
    ...
    meButton.Depth:Set(100)
    meButton:EnableHitTest(true)
    meButton:Show()
    Tooltip.AddButtonTooltip(meButton, 'ToggleMEStorage', 0.2)
end

function ToggleButtons(parent)
    local Group = import('/lua/maui/group.lua').Group
    group = Group(parent)
    LayoutHelpers.AtLeftTopIn(group, parent, 0, 0)
    group.Height:Set(1)
    group.Width:Set(1)
    ...
    local bxpButton = UIUtil.CreateButtonStd(group, '/game/mfd_btn/team-color')
    LayoutHelpers.AtLeftTopIn(bxpButton, group, 220, 60)
    ...
    expandCheck.Depth:Set(110)
    expandCheck:EnableHitTest(true)
    expandCheck:Show()
    group.Depth:Set(100)
    group:EnableHitTest(true)
    group:Show()
end

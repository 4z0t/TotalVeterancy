...
XSL0001 = Class(oldXSL0001) {
    RegenBuffThread = function(self)
        ...
        while not self:IsDead() do
            local units = AIUtils.GetOwnUnitsAroundPoint(self:GetAIBrain(), categories.ALLUNITS, self:GetPosition(),
                RAR + (rmm * math.min(rml, self.VeteranLevel - 1) * RAR))
    ...
    AdvancedRegenBuffThread = function(self)
        ...
        while not self:IsDead() do
            local units = AIUtils.GetOwnUnitsAroundPoint(self:GetAIBrain(), categories.ALLUNITS, self:GetPosition(),
                ARA + (rmm * math.min(rml, self.VeteranLevel - 1) * ARA))
            for _, unit in units do
    ...
    CreateEnhancement = function(self, enh)
        ...
        if enh == 'RegenAura' then
            ...
            table.insert(self.ShieldEffectsBag,
                CreateAttachedEmitter(self, 'XSL0001', self:GetArmy(),

	...
	OuterRingDamage = function(self)
		...
		if self.NukeOuterRingTotalTime == 0 then
			...
			for i = 2, self.NukeOuterRingTicks do
				DamageRing(self:GetLauncher(), myPos, ringWidth * (i - 1), ringWidth * i, self.NukeOuterRingDamage, self.DamageType,
					true, true)
				WaitSeconds(tickLength)
			end
		end
	...
	InnerRingDamage = function(self)
		...
		if self.NukeInnerRingTotalTime == 0 then
			...
			for i = 2, self.NukeInnerRingTicks do
				DamageRing(self:GetLauncher(), myPos, ringWidth * (i - 1), ringWidth * i, self.NukeInnerRingDamage, self.DamageType,
					true, true)
				WaitSeconds(tickLength)
			end
		end

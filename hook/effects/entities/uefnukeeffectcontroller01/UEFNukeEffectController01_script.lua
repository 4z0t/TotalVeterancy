...
UEFNukeEffectController01 = Class(NullShell) {
    ...
    OuterRingDamage = function(self)
        ...
        if self.NukeOuterRingTotalTime == 0 then
            DamageArea(self:GetLauncher(), myPos, self.NukeOuterRingRadius, self.NukeOuterRingDamage, self.DamageType,
                true, true)
        else
            ...
            for i = 2, self.NukeOuterRingTicks do
                DamageRing(self:GetLauncher(), myPos, ringWidth * (i - 1), ringWidth * i, self.NukeOuterRingDamage,
                    self.DamageType, true, true)
                WaitSeconds(tickLength)
            end
        end
    end
    ...
    InnerRingDamage = function(self)
        ...
        if self.NukeInnerRingTotalTime == 0 then
            DamageArea(self:GetLauncher(), myPos, self.NukeInnerRingRadius, self.NukeInnerRingDamage, self.DamageType,
                true, true)
        else
            ...
            for i = 2, self.NukeInnerRingTicks do
                DamageRing(self:GetLauncher(), myPos, ringWidth * (i - 1), ringWidth * i, self.NukeInnerRingDamage,
                    self.DamageType, true, true)
                WaitSeconds(tickLength)
            end
        end
    ...
    CreateInitialFireballSmokeRing = function(self)
        ...
        for i = 0, (sides - 1) do
            ...
            self:CreateProjectile('/effects/entities/UEFNukeShockwave01/UEFNukeShockwave01_proj.bp', X * OffsetMod, 1.5,
                Z * OffsetMod, X, 0, Z):SetVelocity(velocity):SetAcceleration(-0.5)
        end
    end
    ,
    CreateOuterRingWaveSmokeRing = function(self)
        ...
        for i = 0, (sides - 1) do
            ...
            local proj = self:CreateProjectile('/effects/entities/UEFNukeShockwave02/UEFNukeShockwave02_proj.bp',
                X * OffsetMod, 2.5, Z * OffsetMod, X, 0, Z):SetVelocity(velocity)
    ...
    CreateHeadConvectionSpinners = function(self)
        ...
        for i = 0, (sides - 1) do
            ...
            local proj = self:CreateProjectile('/effects/entities/UEFNukeEffect03/UEFNukeEffect03_proj.bp', x,
                HeightOffset, z, x, 0, z):SetVelocity(velocity)
            table.insert(projectiles, proj)
        end
        WaitSeconds(1)
        for i = 0, (sides - 1) do
    ...
    CreateGroundPlumeConvectionEffects = function(self, army)
        ...
        for i = 0, (sides - 1) do
            ...
            self:CreateProjectile('/effects/entities/UEFNukeEffect05/UEFNukeEffect05_proj.bp', x,
                RandomFloat(outer_lower_height, outer_upper_height), z, x, 0, z):SetVelocity(x * velocity, 0,
                z * velocity)
        end
    end
    , }
